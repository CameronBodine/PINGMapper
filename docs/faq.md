...
If you are troubleshooting installation, launch, testing, or export problems, start with the [Troubleshooting](./Troubleshooting.md) page. The FAQ below is intended for stable questions about compatibility, hardware, and data collection.
{: .fs-6 .fw-300 }
...
See [Troubleshooting: ModuleNotFoundError](./Troubleshooting.md#modulenotfounderror).
...
## Does the screen resolution impact the quality or resolution of the data stored in a sonar recording?

No, the screen resolution has no impact on the quality or resolution of the sonar data stored in a sonar recording. How do we know this? Let's take the sonar system I have been using recently. It is a [Solix 12 Chirp Mega SI+ G2](https://humminbird.johnsonoutdoors.com/us/shop/fish-finders/discontinued/solix-12-chirp-mega-si-g2). From the specs on the website, we can see that the screen width is 1280 pixels and a height of 800 pixels. While viewing the fullscreen side scan image, half of those pixels, or 640 pixels, are used to display either side scan channel. If this resolution had an impact on the resolution of the data stored in the recording, we would expect a maximum of 640 returns for each side scan ping. 

PING-Mapper will [export the attributes](./gettingstarted/Exports.md/#ping-attributes-to-csv) for each ping. For a given recording processed with PING-Mapper, this file is in the `meta` folder and called `B002_ss_port_meta.csv` or `B003_ss_star_meta.csv` The number of returns for each ping is stored in the `ping_cnt` column. So if the screen resolution has an impact on the number of returns, we should expect this to be 640, right? Well, it likely is not! It could be 770, 1530, 1212, or any other value. The number of returns is actually directly proportional to the range set while collecting the recording. The higher the range, the more returns are stored in the recording!

Not convinced? Try this simple experiment. Go out and collect two recordings. On the first recording, set the range to a relatively small number, like 20 meters per side. You don't need a lot of data, 30 seconds or so will suffice. Now collect a second recording with a much larger range, say, 200 meters. Process both of these recordings with PING-Mapper. Open up the csv's a take a look at the `ping_cnt` column. There should be much fewer returns in the recording with 20 m range compared to the 200 m range. This will directly translate to the swath width of the rectified imagery. Hope this helps convince you!